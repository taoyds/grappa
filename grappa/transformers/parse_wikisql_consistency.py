...
def write_to_file(sql_data, tables, output_file, replace_q_sql, add_values):
    ...
    unique_labels = set()
    skip_count = 0
    sql_data_10000 = sql_data[:10000]
    sql_data_other = sql_data[10000:]
    nl_sql_pair_strs = []
    for tn, sql_one in enumerate(sql_data_10000):
        if tn % check_point == 0:
        ...
        sql_str = generate_sql_str(sql_one['sql'], columns).lower()
...
#             question = sql_str
        ...
        sql_col_str = "<s> " + sql_str + " </s> " + " </s> ".join(columns).strip() + " </s> "
        nl_sql_pair_str = q_col_str + " ||| " + sql_col_str
        nl_sql_pair_strs.append(nl_sql_pair_str)
        caption = q_col_str + " ||| " + label_str + " ||| " + nl_sql_pair_str
        tokens = tokenizer.tokenize(q_col_str)
        tokens_nl = tokenizer.tokenize(nl_sql_pair_str)
        if len(tokens) > MAX_TOKEN_LEN or len(tokens_nl) > MAX_TOKEN_LEN*2-5:
            skip_count += 1
            continue
        valid_count += 1
        table_file.write(caption.strip().lower().replace("\n", ""))
        #add column names in another new line
        table_file.write("\n")
        
    nl_sql_pair_strs = nl_sql_pair_strs * 6
    
    for tn, sql_one in enumerate(sql_data_other):
        if tn % check_point == 0:
            print("processed: ", str(round(tn/num_sql, 2)))
        columns = tables[sql_one['table_id']]["header"]
        columns = [col if len(col.split(" ")) < 10 else " ".join(col.split(" ")[:5]) for col in columns]
        ...
        nl_sql_pair_str = nl_sql_pair_strs[tn]
        sql_col_str = "<s> " + sql_str + " </s> " + " </s> ".join(columns).strip() + " </s> "
        caption = sql_col_str + " ||| " + label_str + " ||| " + nl_sql_pair_str
        tokens = tokenizer.tokenize(sql_col_str)
        tokens_nl = tokenizer.tokenize(nl_sql_pair_str)
        if len(tokens) > MAX_TOKEN_LEN or len(tokens_nl) > MAX_TOKEN_LEN*2-5:
            skip_count += 1
            continue
...
if __name__ == "__main__":
    main("../data/wikisql", "data/wikisql_10000_consistency.txt")
