...
def write_to_file(sql_data, tables, output_file, replace_q_sql, add_values):
    ...
    for tn, sql_one in enumerate(sql_data):
        ...
        column_str = "COLUMNS: " + " | ".join(columns).strip().lower() + "\n"
        question_str = "QUESTION: " + question.lower() + "\n"
        sql_str = "SQL: " + sql_str.lower() + "\n"
        valid_count += 1
        
        table_file.write(column_str)
        table_file.write(question_str)
        table_file.write(sql_str)
...
if __name__ == "__main__":
    main("../data/wikisql", "data/wikisql_question_sql.txt")
