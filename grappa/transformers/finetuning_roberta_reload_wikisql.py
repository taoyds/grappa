...
from transformers.modeling_roberta_wikisql import RobertaForMaskedLM, RobertaConfig
...
#     print("\n ==============all state_dict in reloaded model: ", state_dict.keys())
    for key in state_dict.keys():
        ...
        if 'roberta.' in key:
            ...
            new_key = 'roberta.' + key
        if "decoder_col" in key:
            new_key = "decoder_col"
        elif "bias_col" in key:
            new_key = "bias_col"
        if new_key:
            old_keys.append(key)
...
def load_roberta(model_path):
    ...
    model_bert = RobertaForMaskedLM(bert_config)
...
def main():
    ...
    parser.add_argument("--bert_model", default=None, type=str, required=False,
                        help="Bert pre-trained model selected in the list: roberta-base, "
                             "roberta-base, roberta-large, reload")
    parser.add_argument("--reload_model", default=None, type=str, required=False,
                        help="Reload Bert pre-trained model")
...
#         os.makedirs(args.output_dir)
    ...
    if args.reload_model:
        model, tokenizer, _ = load_roberta(args.reload_model)
    else:
