...
class RobertaForMaskedLM(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
                masked_lm_labels=None, masked_col_labels=None, input_nl_ids=None, input_nl_mask=None, masked_col_nl=None, input_sql_ids=None, input_sql_mask=None, masked_col_sql=None):
        ...
        lm_prediction_scores, col_prediction_scores = self.lm_head(sequence_output)
        
        outputs_nl = self.roberta(input_nl_ids,
                                  attention_mask=input_nl_mask,
                                  token_type_ids=token_type_ids,
                                  position_ids=position_ids,
                                  head_mask=head_mask)
        sequence_output_nl = outputs_nl[0]
        _, col_prediction_scores_nl = self.lm_head(sequence_output_nl)
        
        outputs_sql = self.roberta(input_sql_ids,
                               attention_mask=input_sql_mask,
                               token_type_ids=token_type_ids,
                               position_ids=position_ids,
                               head_mask=head_mask)
        sequence_output_sql = outputs_sql[0]
        _, col_prediction_scores_sql = self.lm_head(sequence_output_sql)

        # outputs = (prediction_scores,) + outputs[2:]  # Add hidden states and attention if they are here
        if masked_col_labels is not None:
            # TODO: weights for labels
            weight_list = [1.] + [3.]*(LABEL_COUNT-1)
            weights = torch.tensor(weight_list).cuda()
            weighted_loss_fct = CrossEntropyLoss(weight=weights, ignore_index=-1)
            masked_col_loss = weighted_loss_fct(col_prediction_scores.view(-1, LABEL_COUNT), masked_col_labels.view(-1))
        ...
        tgt_col_masked = torch.masked_select(col_prediction_scores_nl.detach(), masked_col_nl.unsqueeze(2)).view(-1, LABEL_COUNT)
        sql_col_masked = torch.masked_select(col_prediction_scores_sql, masked_col_sql.unsqueeze(2)).view(-1, LABEL_COUNT)
#         assert tgt_col_masked.size()[0] == sql_col_masked.size()[0]
        log_softmax = torch.nn.LogSoftmax(dim=-1)
        sql_log_probs = log_softmax(sql_col_masked)
        tgt_log_probs = log_softmax(tgt_col_masked)
        kl_loss = kl_for_log_probs(tgt_log_probs, sql_log_probs)
        total_loss += kl_loss
        
        return total_loss, masked_col_loss, kl_loss
...
def kl_for_log_probs(log_p, log_q):
    ...
    kl = torch.mean(neg_ent - neg_cross_ent)
...
class RobertaLMHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.decoder_col = nn.Linear(config.hidden_size, LABEL_COUNT, bias=False)
        self.bias_col = nn.Parameter(torch.zeros(LABEL_COUNT))
    ...
    def forward(self, features, **kwargs):
        ...
        return lm_prediction_scores, col_prediction_scores
