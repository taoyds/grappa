...
class ActionSpaceWalker:
    ...
    """
    def __init__(self, language: WikiSQLLanguage) -> None:
    ...
    def prune_row_selections(self, row_selection_cache: List):
        kg = self._language.table_context._knowledge_graph
        tokens = [t.text for t in self._language.table_context.question_tokens]
        ret_cache = []
        for r_ in row_selection_cache:
            ...
            elif r_lf.startswith("(conjunction"): # conjunction
                r_lf_tree = Tree.fromstring(r_lf)
                if str(r_lf_tree[0]) != str(r_lf_tree[1]):
                    if r_lf_tree[0].label() == "filter_in" and r_lf_tree[0][2] not in kg[r_lf_tree[0][1]]:
                        continue
                    if r_lf_tree[1].label() == "filter_in" and r_lf_tree[1][2] not in kg[r_lf_tree[1][1]]: 
                        continue
                    ret_cache.append(r_)
    ...
    def get_junctions(self, single_selections: List) -> List:
        ...
        for sin_ac in single_selections:
            ...
            # if sin_ac_lf.startswith("(filter_in") or sin_ac_lf.startswith("(filter_number_equals"): 
            if sin_ac_lf.startswith("(filter"): # allow all filter functions
                candidates.append(sin_ac)

        # if too many candidates, the junctions will explode 
    ...
    def get_action_seqs_from_sketch(self,
        ...
        for action_ind, action in enumerate(sketch):
            ...
            if lhs in ["Column", "StringColumn", "NumberColumn", "str", "Number"] and rhs == "#PH#": 
