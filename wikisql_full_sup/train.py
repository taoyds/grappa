...
def train(train_loader, train_table, model, model_bert, opt, bert_config, tokenizer,
    ...
    for iB, t in enumerate(train_loader):
        ...
        cnt_x1_list, g_ans, pr_ans = get_cnt_x_list(engine, tb, g_sc, g_sa, sql_i, pr_sc, pr_sa, pr_sql_i) #[0], [0], [0]
...
#             save_for_evaluation(path_save_for_evaluation, results_dev, 'dev')
            ...
            if acc_lx_t > acc_lx_t_best:
                ...
                torch.save(state, os.path.join(save_model_path, "model_best.pt"))

                state = {'model_bert': model_bert.state_dict()}
                torch.save(state, os.path.join(save_model_path, 'model_bert_best.pt'))

            if epoch < 50:
                model_name = "model_{}.pt".format(str(epoch))
                state = {'model': model.state_dict()}
    ...
    if args.do_infer:
        ...
        path_db = './data'
        db_name = 'ctable'
        data_table = load_jsonl('./data/ctable.tables.jsonl')
        table_name = 'ftable1'
